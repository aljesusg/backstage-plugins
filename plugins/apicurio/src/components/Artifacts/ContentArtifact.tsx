...
export const ContentArtifact = (props: {artifactContent : string, artifactType: string}) => {  
  const [editorMode, setEditorMode] = React.useState<string>(getEditorMode(props.artifactType));
  const [editorHeight, setEditorHeight] = React.useState<string>("500px");
  const [content, setContent] = React.useState<string>(formatContent(props.artifactContent));
  ...
  const switchJsonYaml = (_: React.MouseEvent<HTMLElement>, mode: string ) => {
        if (mode === editorMode) {
          ...
          try {
              if (mode === "yaml") {
                  content = YAML.stringify(JSON.parse(props.artifactContent), null, 4);
              } else {
                  content = JSON.stringify(YAML.parse(props.artifactContent), null, 2);
              }
          } catch (e) {
              console.log("[Content] Invalid content error:", e)
          }
          setEditorMode(mode)
          setContent(content)
      }
