...
export class ApicurioApiImpl implements ApicurioApi {
  ...
  async searchArtifacts(query: string): Promise<ArtifactsSearchResults> {
    try {
      const searchArtifacts = this.fetch<ArtifactsSearchResults>(
  ...
  async getArtifact(
    ...
    try {
      ...
      let rules: string[] = [];
      const metaArtifact = this.fetch<ArtifactMetaData | ApiError>(
        `${endpointVersion}/meta`,
        'getMetaArtifact',
      );
      const contentArtifact = this.fetch<string | ApiError>(
        endpointVersion,
        ' getContentArtifact',
        'text',
      );
      const rulesArtifact = this.fetch<string[] | ApiError>(
        `${endpoint}/rules`,
        ' getRulesArtifact',
      );
      const versionsArtifact = this.fetch<SearchedVersionResults | ApiError>(
